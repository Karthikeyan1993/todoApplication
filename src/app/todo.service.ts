...
export class TodoService {
  ...
  getAllTodos = (param?: any): Observable<Todo[]> => {
    return this.http.get<Todo[]>(AppSettings.APP_BASE_URL + `${this.ALL_TODO_URL}`)
  ...
  saveTodo = (param?: TodoRequest): Observable<Todo> => {
    return this.http.post(AppSettings.APP_BASE_URL + `${this.SAVE_TODO_URL}`, param)
      .pipe(map((res: Todo) => res),
        catchError((error) => {
  ...
  updateTodo = (param?: Todo): Observable<Todo> => {
    return this.http.put(AppSettings.APP_BASE_URL + `${this.UPDATE_TODO_URL}`, param)
      .pipe(map((res: Todo) => res),
        catchError((error) => {
  ...
  deleteTodo = (param?: string) => {
    return this.http.delete(AppSettings.APP_BASE_URL + `${this.DELETE_TODO_URL}` + param)
      .pipe(map((res) => res),
        catchError(error => {
  ...
  findTodo = (param?: string): Observable<Todo> => {
    return this.http.get(AppSettings.APP_BASE_URL + `${this.FIND_TODO_URL}` + param)
      .pipe(map((res: Todo) => res),
        catchError((error) => {
